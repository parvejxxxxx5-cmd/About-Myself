# About-Myself
# Hey 👋, I'm Parvej Alam  
**IIT Madras BS Data Science & Applications '29** | 4 AM warrior | Skills >> Certificates

![IIT Madras](https://img.shields.io/badge/IIT%20Madras-BS%20Data%20Science%20%26%20Applications-blue?style=for-the-badge)
![Python](https://img.shields.io/badge/Python-Expert-3776AB?style=for-the-badge&logo=python)
![SQL](https://img.shields.io/badge/SQL-Master-000000?style=for-the-badge&logo=postgresql)
![Power%20BI](https://img.shields.io/badge/Power%20BI-Guru-F2C811?style=for-the-badge&logo=powerbi)
![Machine%20Learning](https://img.shields.io/badge/ML-Andrew%20Ng%20Specialization-FF6F00?style=for-the-badge&logo=coursera)

### Current Focus (Sep 2025 → May 2026)
- Finish Google Data Analytics Professional Certificate (6/8 done ✅)
- Complete Andrew Ng Machine Learning Specialization (Stanford + DeepLearning.AI)  
- Become absolute monster in Excel → SQL → Power BI (PL-300 ready)
- IIT Madras Year-1 → 8+ CGPA locked
- 10+ end-to-end projects (Banking | Retail | Healthcare | Predictive models)
- Paid Data Analyst / Junior ML internship before May 2026 → non-negotiable

### Certificates I’m Stacking
- Google Data Analytics Professional Certificate (Dec 2025)
- Andrew Ng Machine Learning Specialization (Mar 2026 target)
- Internshala Python + AI → National Top Performer + NSDC
- Microsoft Power BI Data Analyst Associate (PL-300) → in progress
- ISP 53 Appreciation – Internshala Student Partner
- Tata Crucible Campus Quiz – Prelims Cleared
- More dropping every 30–45 days

### Technical Skills (Mastered or Mastering)
```text
Python      • SQL            • Excel (Power Query + DAX)
Power BI    • Tableau        • Pandas        • NumPy
Scikit-learn• Statistics      • Hypothesis Testing
Regression  • Neural Networks • Git           • GitHub
